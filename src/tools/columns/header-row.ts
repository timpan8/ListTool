...
export const headerRowTool: Tool = {
  ...
  run(input, options) {
    ...
    const header: Row = makeRow(
      0,
      Object.fromEntries(input.columns.map((column) => [column.id, column.name])),
    );
    const rows = [header, ...input.rows.map((row, index) => makeRow(index + 1, row.cells))];
